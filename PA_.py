...
class MultiLayerPerceptron():
    ...
    def attack(self,x,y, num_iter):
        ...
        #######################################
        alpha = self.eps / num_iter  #step size
        x_adv = x.copy()               #initialize adversarial example

        for _ in range(num_iter):
            self.predict(x)
            x_adv = x_adv + alpha * np.sign(self.gradient(x_adv, y))
        #######################################
        return x_adv
...
for eps in epss:
    ...
    for i in range(nTest):
        ...
        Y_pred[i] = clf.predict(clf.attack(x, y, num_iter=10))
